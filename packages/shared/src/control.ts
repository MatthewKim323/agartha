 ...
 * mcp-server depends on @agartha/shared (this interface), never on the app.
 */
export interface BotControl {
  ...
  /** Fell a whole tree near a coord: mine the trunk column + branches, standing
   * under it and mining upward as it clears. Returns logs mined. */
  fellTree(near: Vec3Lit): Promise<number>;

  /** Dig a descending 1x2 staircase in the bot's facing direction (for going
   * down to mine stone). Stops at lava/bedrock. Returns blocks mined. */
  mineStaircase(steps?: number): Promise<number>;
  ...
  /** Craft with prerequisites auto-resolved (logs→planks→sticks) + table placed. */
  craftSmart(itemName: string, count?: number): Promise<void>;
