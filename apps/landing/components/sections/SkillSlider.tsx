...
const SKILLS = [
  {
    ...
    body: 'agar finds the nearest tree, takes the whole thing down, and picks up what falls. Setting off happens the moment you ask, so "aight, otw" reaches you from halfway there.',
  },
  {
    name: 'Mine down',
    file: 'mine-down.ts',
    kind: 'mine' as SkillKind,
    body: 'agar cuts a staircase rather than dropping straight down, watching for lava the whole way. That check runs in plain code, because you do not want a language model deciding whether that is lava.',
  },
  {
    name: 'Fetch item',
    file: 'fetch-item.ts',
    kind: 'fetch' as SkillKind,
    body: 'agar goes and gets an item, from a chest or off the ground, and brings it back. You hear "done" once agar is actually holding it, not on setting off.',
  },
  {
    name: 'Combat assist',
    file: 'combat-assist.ts',
    kind: 'combat' as SkillKind,
    body: 'agar steps in when something hostile gets close, and backs off when nothing is. This runs on the fast loop, quicker than anything waiting on a model could manage.',
  },
  {
    name: 'Scout ahead',
    file: 'scout-ahead.ts',
    kind: 'scout' as SkillKind,
    body: 'agar runs ahead and tells you what is out there. The useful part: describing the view happens without stopping to do it.',
  },
  {
    name: 'Craft',
    file: 'craft.ts',
