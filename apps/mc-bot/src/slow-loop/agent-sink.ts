...
function buildPrompt(reason: string, state: GameState): string {
  return [
    ...
    "",
    "How to act:",
    "- This snapshot is stale by the time you read it. Before acting, read the `agartha://state/current` resource for live state, and `agartha://memory/world` for what itto already knows about this world (waypoints, chest contents).",
